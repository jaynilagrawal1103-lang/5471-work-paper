...
## Session 2026-09-14 — OCR before processing: PaddleOCR service, auto-detect at upload, gate, provenance

The OCR path used to run AFTER a failed processing pass (scan reaches the
reader → "no text layer" → watcher → Tesseract.js in the browser → copy
attached → re-process), with every figure of the first pass wrong or missing
and nothing saying so until the second. Now OCR runs at upload, through a
separate engine service, and processing waits for it.

**`ocr-service/` (Python, FastAPI, :8472).** PaddleOCR 3.x is the primary
engine — PP-OCRv6 when the installed release and the local model store provide
it (3.7.0 lists v6), stepping down to PP-OCRv5; PP-StructureV3 for table pages.
On a host that cannot reach a model hoster the native pipeline is skipped
WITHOUT waiting on hoster timeouts (checked up front from `~/.paddlex` /
`OCR_PADDLE_MODEL_DIR`) and PP-OCRv5 runs through ONNX Runtime from the
weights the `onnxocr` wheel ships. Surya is the second reading when installed
(0.22: `RecognitionPredictor(SuryaInferenceManager)`, blocks with polygons —
split proportionally into words); Tesseract is the last fallback and, when
Surya is absent, the cheap second opinion on numeric tokens (crops, psm 7).
Per page: render 300 dpi → orientation (Tesseract OSD, else projection
heuristic) → deskew ±8° by projection-profile search (Hough was fooled by
noise) → denoise only when grainy (residual std > 5) → CLAHE → recognise →
...
by box overlap, reported only when the alternative itself parses as a number
(Tesseract crop junk like `11, aC` is not a disagreement). Nothing is ever
corrected in place. Endpoints accept multipart or a raw body + query string —
the browser and both app servers use the raw form. `tests/` (pytest, 25) run
the real engines that load; fixtures regenerated by `tests/make_fixtures.py`
into `tests/fixtures/ocr/` (digital, scanned, mixed, difficult, multipage,
table).

**Servers.** `server/src/ocrProxy.ts` (Fastify) and `scripts/serve-local.mjs`
both forward `/api/ocr/{health,detect,ocr}` to `OCR_SERVICE_URL` (default
`http://127.0.0.1:8472`), raw body through; `/api/health` gains `ocr`.

**Store (both trees).** `EntityFile.ocr` sidecar type; bridge actions
`EN9_probePdf` (per-page text presence with the app's own pdf.js) and
`EN9_replaceFile` (swap in place, NEW id because persistence keys blobs by id,
overrides migrated, original name + SHA on the sidecar); `processEntity`
awaits `globalThis.EN9OCRGATE.wait(id)` FIRST when `pending(id)` — status
"processing"/progress −1 while waiting, back to idle with a toast if OCR
fails; `ocrReviewItems` raised in step 1 for every file with a sidecar
BEFORE the read, so a failed page blocks even when the copy will not open;
`ocrProvenance` appends OCR engine / confidence / position columns to the
Provenance sheet and an OCR DOCUMENTS section; `readState` returns
"text read (OCR)". Dist sentinels: `EN9OCRHELPERS` (before `var Be={`),
`EN9OCRACT`, `EN9OCRGATE`, `EN9OCRFLAGS` (inside `try{` before `_v(y.blob)`),
`EN9OCRPROV` (three patches in `EN9prov`), and the `EN9RDPATH` block.

**Layer (`layer-src/enhance.js`, re-injected).** `EN9OCR.service` client
(health cached 60 s, raw-body posts), `EN9OCRGATE`, `EN9ocrIntakeTick` (probe
every new PDF on `wp:state`, start a job for the scanned pages with an
explicit page list), `EN9ocrStart` (mark gate → service → fallback →
`EN9_replaceFile` → release; failure releases with the error and keeps the
original attached), `en9OcrRun` keeps the `EN9OCR.io` seam and now copies
non-target pages from the source PDF (pdf-lib `copyPages`) so a mixed PDF
keeps its digital text under the fallback too. The old `EN9AUTOOCR` watcher
stays as a safety net and skips files the new path already handled. The OCR
card gained a page-range box, a force checkbox and a live engine line; the
Settings card describes the chain. `fetch` is wrapped in a resolved promise so
a DOM without it (tests, file://) reads as "not reachable", never an exception
out of the watcher.

**Tests.** `tests/test_ocr_src.cjs` (12) boots the source store: probe on the
fixtures, replace, review items, provenance, the gate (waits / rejects / absent).
`tests/test_ocr_workflow.cjs` (11) checks the dist sentinels, evaluates the
dist helpers against the same input as src, drives the shipped layer with the
service stubbed silent (fallback path, gate holds until the slow stub returns)
and online (raw body, `pages=2` for the mixed PDF, sidecar from the response),
and a failure while Process Entity waits. jsdom cannot run the bundled pdf.js
text layer (no structuredClone/TextDecoder/font pipeline), so the shipped
reader's probe is stubbed there and verified for real by
`tests/e2e/ocr_e2e.mjs` — Chromium against `serve-local` + the service, all
six fixtures: upload → auto job → Process pressed while OCR runs (waits) →
replaced copy with sidecar → figures booked → review notice → generated
workbook's Provenance cites the engine. Not in `test:all` (needs the service);
`npm run test:e2e-ocr`. `test:all` is now 56 suites / 1,367 assertions.

**Known limits.** Native PaddleOCR weights need one online run (or a copied
`~/.paddlex`); this sandbox could not reach any hoster, so the real-engine
tests here ran PP-OCRv5 (ONNX) + Tesseract 5.3.4 and the Surya adapter was
verified only as "installed, models absent → unavailable, chain moves on".
The e2e fixtures' balance sheet does not tie under the mapping rules
(liabilities section unbooked), so the e2e acknowledges the balance blocker
with a note before generating — that is the app's own documented path. The
trial-balance fixture (`table.pdf`) is OCR'd cleanly (63 tokens at 0.99) but
