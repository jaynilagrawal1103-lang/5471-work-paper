...
function book(items, opts = {}) {
  ...
  for (const it of feed) {
    ...
    /* A payment processor's fee is an ordinary deduction unless the statement
       files it under cost of sales — the same override the booking loop
       applies, replicated here so the fixtures measure the shipped rule. */
    if (it.feed === "is" && it.section === "cogs" && target === "IS:OD" && ENG.isProcessorFee(norm.label)) target = "IS:12";
    ...
    if (SECT.contraRevenueFlip(alloc.target, it.inTotal, rowIn.values[0])) rowIn.values = rowIn.values.map((v) => -v);
    if (SECT.expenseGainFlip(alloc.target, it.section, rowIn.values[0])) rowIn.values = rowIn.values.map((v) => -v);
